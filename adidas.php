<?php
include 'koneksi.php';
$adidass= mysqli_query($conn, "SELECT id_sepatu, img, nama_sepatu, harga, stok 
    FROM sepatu
    JOIN merk ON sepatu.id_merk = merk.id_merk
    WHERE merk.nama_merk = 'Adidas'");
?>
<!DOCTYPE html>
<html lang="en">
                        ...
                        <div class="col-md-4">
                            <div class="card mb-4 product-wap rounded-0">
                                <div class="card rounded-0">
                                    <img class="card-img rounded-0 img-fluid" src="image/<?= $adidas['img']; ?>" width="300" height="360">
                                    <div class="card-img-overlay rounded-0 product-overlay d-flex align-items-center justify-content-center">
                                        <ul class="list-unstyled">
                                            <li><a class="btn btn-success text-white mt-2" href="detail.php?id=<?= $adidas['id_sepatu']; ?>"><i class="far fa-eye"></i></a></li>
                                        </ul>
                                    </div>
                                </div>
                                <div class="card-body">
                                    <ul class="list-unstyled d-flex justify-content-between">
                                        <li class="text text-left">Rp <?= number_format($adidas['harga'], 0, ',', '.'); ?></li>
                                    </ul>
                                    <a href="detail.php?id=<?= $adidas['id_sepatu']; ?>" class="h2 text-decoration-none text-dark"><?= $adidas['nama_sepatu']; ?></a>
                                    <p>Stok (<?= $adidas['stok']; ?>)</p>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
